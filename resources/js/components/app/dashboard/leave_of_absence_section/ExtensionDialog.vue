<template>
  <v-dialog v-model="is_active" scrollable persistent max-width="1000">
    <v-card>
      <v-card-title>
        <h4 class="ca-typo-title-4">Students who are on leave of absence on {{ $moment().format('M/D/Y') }}</h4>
        <v-spacer></v-spacer>
        <div>
          <el-input
            v-model="search"
...
export default {
  ...
  computed: {
    full_leaves_today () {
      return this.$store.getters['dashboard/full_leaves_today']
    }
  },
  watch: {
    is_active (new_v) {
      if (new_v) {
        this.$store.dispatch('dashboard/get_full_leaves_today').then(res => {
          /*
  ...
  created () {
    bus.$on('dialog:dashboard.full_leaves_today', (payload) => {
      this.is_active = true
    })
  }
}
