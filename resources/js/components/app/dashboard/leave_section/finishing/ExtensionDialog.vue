<template>
  <v-dialog v-model="is_active" scrollable persistent max-width="1000">
    <v-card>
      <v-card-title>
        <h4 class="ca-typo-title-4">Students who will finish leaves</h4>
        <v-spacer></v-spacer>
        <div>
          <el-input
            v-model="search"
...
export default {
  ...
  computed: {
    full_finishing_leaves () {
      return this.$store.getters['dashboard/full_finishing_leaves']
    }
  },
  watch: {
    is_active (new_v) {
      if (new_v) {
        this.$store.dispatch('dashboard/get_full_finishing_leaves').then(res => {
          /*
  ...
  created () {
    bus.$on('open_dialog_for_finishing_leaves_extension', (payload) => {
      this.is_active = true
    })
  }
}
